...
double* consultComittee(struct fann_train_data *data, struct fann ** nets, double * committee_decision, int num_nets){
  int i, j, k;
...
void evaluate_committee(struct fann_train_data *data, struct fann ** nets, double * committee_decision, int num_nets){
  printf("evaluate");
}
...
int main(int argc, char *argv[])
{
  ...
  double *committee_decision = NULL;
  int i, numNets = argc-2;

  data = checkArgs(argc, argv, data);
  nets = populateNets(numNets, argv, nets);

  committee_decision = consultComittee(data, nets, num_nets);
  evaluate_committee(data, nets, committee_decision, num_nets);
  for (i = 0; i < numNets; i++)
    fann_destroy(nets[i]);
  free(nets);
