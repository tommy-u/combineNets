...
//	const unsigned int input = 2, hid = 2, out = 2, layers = 3;
	...
	dnn = init(layers, input, 4, out, cnn);
	enn = init(layers, input, 4, out, cnn);
	...
	fann_train_on_file(dnn, "x.data", max_epochs, epochs_between_reports, desired_error);
	fann_train_on_file(enn, "x.data", max_epochs, epochs_between_reports, desired_error);
